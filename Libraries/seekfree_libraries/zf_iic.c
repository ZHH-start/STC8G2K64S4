...
uint8 iic_write_reg(uint8 dev_add, uint8 reg, uint8 dat)
{
    ...
    //        return IIC_SEND_FAIL;
    if (send_data(dev_add) != IIC_SEND_OK)
        return IIC_SEND_FAIL;
...
//  连续写数据
char iicWriteDate(unsigned char devAddr, unsigned char regAddr, unsigned char *p_dat, int len)
{
    start();            // 发送起始命令
    send_data(devAddr); // 发送设备地址+写命令
    recv_ack();
    send_data(regAddr); // 发送存储地址高字节
    recv_ack();
    while (len--) {
        send_data(*p_dat++); // 写测试数据1
        recv_ack();
    }
    stop();
    return 0;
